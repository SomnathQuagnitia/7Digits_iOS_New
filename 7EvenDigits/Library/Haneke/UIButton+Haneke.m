...
- (void)hnk_setImageFromFetcher:(id<HNKFetcher>)fetcher forState:(UIControlState)state placeholder:(UIImage*)placeholder success:(void (^)(UIImage *image))successBlock failure:(void (^)(NSError *error))failureBlock
{
    for (int i = 0; i < [[self subviews] count]; i++)
    {
        ...
        {
            UIActivityIndicatorView *UIActivityIndicatorViewObj = (UIActivityIndicatorView *)[[self subviews] objectAtIndex:i];
            [self setBackgroundColor:[UIColor clearColor]];
            [UIActivityIndicatorViewObj removeFromSuperview];
            UIActivityIndicatorViewObj = nil;
        }
    }
...
- (void) addLoadingIndicator
{
    ...
    CGFloat left = (self.frame.size.width - act.frame.size.width) / 2.0;
    CGFloat top = (self.frame.size.height - act.frame.size.height) / 2.0;
    act.frame = CGRectMake(left, top, act.frame.size.width, act.frame.size.height);
    act.tag = 1999;
    [self addSubview:act];
}

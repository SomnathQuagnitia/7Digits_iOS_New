...
@interface ViewProfessionalProfileVC ()

@end

@implementation ViewProfessionalProfileVC
@synthesize professionalProfileArray,fullNameLabel,userIdLabel,notesDetailWebView,titleLabel,companyLabel,userImage,digitsButton,addressTextView,viewForProfileData,playButton,videoUrl,locationTxtLbl,moviePlayer,player,playerItem;
...
- (void)viewDidLoad
{
    ...
    [profileScrollView setContentSize:CGSizeMake(0, 720)];

    if ([stringForTitle isEqualToString:@"Professional Profile"])
    {
        [viewForProfileData setFrame:CGRectMake(0, 80, 320, 311)];
        // self.notesDetailWebView.frame = CGRectMake(self.notesDetailWebView.frame.origin.x, 80+375, self.notesDetailWebView.frame.size.width, self.notesDetailWebView.frame.size.height);
    ...
    else
    {
        ...
        //self.notesDetailWebView.frame = CGRectMake(self.notesDetailWebView.frame.origin.x, 378, self.notesDetailWebView.frame.size.width, self.notesDetailWebView.frame.size.height);
    ...
    rightBarBtn = [[UIBarButtonItem alloc]initWithTitle:@"Update" style:UIBarButtonItemStylePlain target:self action:@selector(handelUpdate)];
...
-(void)setProfessionalProfileData
{
    ...
    NSMutableString *state,*city;
    city = [[NSMutableString alloc] initWithString:[[[professionalProfileArray objectAtIndex:0] valueForKey:@"userDTO"] valueForKey:@"City"]];
    state  =[[[professionalProfileArray objectAtIndex:0] valueForKey:@"userDTO"] valueForKey:@"State"];
    ...
    [city appendFormat:@" , %@",state];
    self.locationTxtLbl.text = city;
    ...
    else
    {
        ...
        [self.addressTextView setFont:[UIFont systemFontOfSize:20]];
        ...
        if (str.length > 0)
        {
            ...
            if (arr.count > 0)
            {
                if (arr.count == 1)
                {
                    ...
                    {
                        [addressString appendFormat:@"%@",[NSString stringWithFormat:@"%@",[arr objectAtIndex:0]]];
                    }
                }
                else if (arr.count == 2)
                {
                ...
                else if (arr.count == 3)
                {
                    ...
                    if (![[arr objectAtIndex:1] isEqualToString:@"Address2"]) {
                        [addressString appendFormat:@"%@  ",[NSString stringWithFormat:@"%@",[arr objectAtIndex:1]]];
                    }
...
- (void)webViewDidFinishLoad:(UIWebView *)webView
{
    ...
    [profileScrollView setContentSize:CGSizeMake(0, 570+height)];
...
-(void)callCheckUserExistOrNotWebService
{
    ...
    else
    {
        ...
        [serviceIntegration CheckProfileUserExist:self UserName:searchUserName :@selector(receivedResponseUserExistWS:)];
    }
}
...
//        [[player currentItem] removeObserver:self forKeyPath:@"rate"];
//
